...
class FastSAMPrompt:
    ...
    def plot_to_result(self,
        ...
        if withContours:
            ...
            # color = np.array([0 / 255, 0 / 255, 255 / 255, 0.8]) # 파란색
            color = np.array([255/255, 165/255, 0/255, 0.8]) # 주황색
        ...
        #     buf = fig.canvas.tostring_rgb()
        try:
            buf, (cols, rows) = fig.canvas.print_to_buffer()  # 버퍼와 크기 정보 가져오기
            buf = np.frombuffer(buf, dtype=np.uint8).reshape(rows, cols, 4)  # ARGB 형식
            # buf = buf[:, :, 1:].copy(order='C')  # ARGB → RGB 변환 (A 채널 제거)
            buf = buf[:, :, :3].copy(order='C') 
        except AttributeError:
            fig.canvas.draw()
            buf, (cols, rows) = fig.canvas.print_to_buffer()
            buf = np.frombuffer(buf, dtype=np.uint8).reshape(rows, cols, 4)
            # buf = buf[:, :, 1:].copy(order='C')  # A 채널 제거
            buf = buf[:, :, :3].copy(order='C') 
    ...
    def fast_show_mask(
        ...
        mask_count = annotation.shape[0]
        height = annotation.shape[1]
        width = annotation.shape[2]

        # 마스크 크기 순으로 정렬 (큰 객체 먼저 그리기)
        ...
        # 🎨 마스크 색상을 파란색(BGR)으로 고정
        # color = np.array([0, 0, 255]) / 255.0  # 파란색 (RGB 기준)
        color = np.array([255, 165, 0]) / 255.0  # 주황색 (RGB)
        
        transparency = 0.6  # 투명도 설정
        visual = np.concatenate([color.reshape(1, 1, 3), np.array([[transparency]])], axis=-1)  # 색상 + 투명도
        mask_image = np.expand_dims(annotation, -1) * visual

        show = np.zeros((height, width, 4))  # RGBA 채널 추가
        ...
        # 🟢 배경을 원본 이미지로 유지
        background = cv2.cvtColor(self.img, cv2.COLOR_BGR2RGBA) / 255.0  # 배경 원본 유지
        alpha_mask = show[:, :, 3:]  # 투명도 채널
        show = show[:, :, :3] * alpha_mask + background[:, :, :3] * (1 - alpha_mask)  # 마스크 적용된 부분만 강조
    ...
    def fast_show_mask_gpu(
        ...
        # color = torch.ones((msak_sum, 1, 1, 3), device=annotation.device) * torch.tensor([0.0, 80/255.0, 1.0], device=annotation.device) # 색상 고정 (주황색)
        if random_color:
    ...
    def box_prompt(self, bbox=None, bboxes=None):
        ...
        for bbox in bboxes:
            ...
            masks = self.results[0].masks.data  # FastSAM 모델의 출력 마스크 데이터
            target_height = self.img.shape[0]   # 입력 이미지의 원본 크기
            target_width = self.img.shape[1]    
            h = masks.shape[1]   # FastSAM 세그멘테이션 마스크 크기
            w = masks.shape[2]
